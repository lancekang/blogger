...
### P1. 현재 pending 7건 순차 처리

- 한 실행당 1건 claim
- 공식 제조사 자료와 국내 가격 웹 검증
- 매거진 템플릿 적용
- 성공 시 결과 API 저장
- 실패 시 반드시 `failed` 상태와 원인 기록

### P2. 실제 브라우저 시각 검수

- 다음 원고가 생성되면 데스크톱·모바일 화면부터 확인
- 필요한 경우 템플릿의 간격, 글자 크기, 색상만 조정
- 구조를 다시 평면 레이아웃으로 되돌리지 않음

### P3. 오래된 프로젝트 문서 정리

`CURRENT_PROGRESS.md`와 `.agents/skills/blogger-helper/references/` 일부에는 현재 코드에 없는 RSS, Research, RAWG, entity extraction 모듈이 구현된 것처럼 남아 있을 수 있습니다. 실제 파일 트리 기준으로 갱신해야 합니다.

### P4. 임시 스크립트 정리

`scratch/generate_post.py`에는 절대경로, 특정 요청 ID, 직접 완료 상태 변경 코드가 있습니다. 운영 흐름에서 제외하거나 새 claim/result API 방식으로 변경해야 합니다.

### P5. 고정 연도 콘텐츠 개선

`app/api/ai/trend-keywords/route.ts`의 추천 키워드에는 `2026년`이 하드코딩돼 있습니다. 실행 시점 연도를 사용하도록 변경하는 것이 좋습니다.

### P6. 저장소 상태 확인

이전 세션에서 `.git` 디렉터리는 보였지만 `git status`가 `not a git repository`로 실패했습니다. 실제 Git 메타데이터가 유효한지 확인해야 합니다. Git 저장소가 아니면 Codex worktree 기능은 사용할 수 없습니다.
